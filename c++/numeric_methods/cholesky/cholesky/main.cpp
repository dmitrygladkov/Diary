...
#pragma omp parallel for
	for (int i = 0; i < left_dim1; i++) {
		for (int j = 0; j < right_dim2; j++) {
...
static inline
void Cholesky_Decomposition_recursive(double *A, double *L, int n,
	double *A_full, double *L_full, int n_full);
...
void Cholesky_Decomposition(double *A, double *L, int n)
{
	...
	Cholesky_Decomposition_recursive(A22_red, L22_red, n - BLOCK_SIZE, A, L, n);
	/*size_t k = 0, l = 0;
				...
				double *A_full, double *L_full, int n_full)
{
...

	{
		...
		Cholesky_Decomposition_recursive(A22_red, L22_red, n - BLOCK_SIZE, A_full, L_full, n_full);
	}
}
